 ...
 *
 * Contains the Módulo 11 validation algorithms for both document types and
 * the hooks that enforce them:
 *  - Server-side: woocommerce_blocks_validate_additional_checkout_field
 *    rejects orders with an invalid document before they are created.
 *  - Client-side: a wp_footer script mirrors the same algorithms via
 *    wp.data so the user gets immediate native WooCommerce Blocks feedback
 *    (red border + error message) without a server round-trip.
 *
 * Field registration and show/hide logic live in inc/billing.php.
 *
 * @package libresign
 */
